...
<div class="card mt-2">
    <div class="card-body">
        ...
        while($d = mysqli_fetch_array($data)){
            ...
            <?php if ($d['PelangganID'] == $PelangganID) { ?>
                ...
                <?php 
                // Check if there are any details for this purchase
                $detailpenjualan = mysqli_query($koneksi, "SELECT COUNT(*) AS TotalDetails FROM detailpenjualan WHERE PenjualanID='".$d['PenjualanID']."'");
                $row = mysqli_fetch_assoc($detailpenjualan);
                $totalDetails = $row['TotalDetails'];

                // Check if total harga is not empty
                $totalHarga = $d['TotalHarga'];

                if ($totalDetails > 0 && !empty($totalHarga)) { // Show the button only if there are details and total harga is not empty
                ?>
                <form method="post" action="lihat_nota.php" onsubmit="return openPopup(this);">
                    ...
                    <button type="submit" class="btn btn-success btn-sm mt-1 mb-3">Lihat Nota</button>
                </form>
                <?php } ?>
    ...
    <a href="history.php" class="btn btn-danger">Kembali</a> <!-- Ganti dengan tautan halaman yang sesuai -->
</div>
